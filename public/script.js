...
function addInterest(interest) {
    if (!selectedInterests.includes(interest)) {
        ...
        document.getElementById('doneButton').style.display = 'inline-block'; // Show the Done button
    }
}

// Function to handle the 'Done' button click
function submitInterests() {
    // Hide the interest selection buttons and Done button
    document.getElementById("interestSelection").style.display = "none";
    document.getElementById("doneButton").style.display = "none";

    // Send the selected interests to the backend to store them
    fetch('/api/saveInterests', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        ...
        // Now fetch the categories related to the selected interests
...
// Function to fetch categories based on selected interests
function fetchCategoriesForUser() {
    fetch('/api/categories?interests=' + encodeURIComponent(JSON.stringify(selectedInterests)))
    .then(response => response.json())
    .then(categories => {
